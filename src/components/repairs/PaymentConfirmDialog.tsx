          ...
          {isAlreadyPaid && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-success/10 border border-success/20">
              ...
              <p className="text-sm text-success">
                Cette réparation est déjà soldée. Vous pouvez tout de même enregistrer un paiement supplémentaire si nécessaire.
              </p>
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            {isFullPayment && paymentAmount > 0 ? (
              <div className="flex items-start gap-2 p-3 rounded-lg bg-success/10 border border-success/20">
                <CheckCircle2 className="h-4 w-4 text-success mt-0.5 shrink-0" />
                <p className="text-sm text-success">
                  {remaining === 0 ? `Paiement supplémentaire de ${format(paymentAmount)} enregistré.` : "Réparation entièrement payée."}
                </p>
              </div>
            ) : paymentAmount > 0 ? (
              ...
              <p className="text-xs text-muted-foreground">Aucun paiement supplémentaire à enregistrer.</p>
            )}
