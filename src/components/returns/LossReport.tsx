...
export function LossReport() {
  ...
  const months: { label: string; warranty: number; refunds: number }[] = [];
  for (let i = 5; i >= 0; i--) {
    const d = new Date();
    ...
    months.push({ label, warranty: warrantyCost, refunds: refundCost });
  }

  const totalLoss = months.reduce((s, m) => s + m.warranty + m.refunds, 0);
  const writtenOffParts = (parts as any[]).filter((p: any) => p.status === "written_off" || p.status === "rejected").length;
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
          ...
          <div className="text-center p-3 rounded-lg bg-muted/50">
            <p className="text-xs text-muted-foreground">Pièces perdues</p>
            <p className="text-lg font-bold font-mono-numbers">{writtenOffParts}</p>
          </div>
        </div>
